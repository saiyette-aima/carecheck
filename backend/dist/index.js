...
app.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET + user.password, { expiresIn: "30m" });
...
app.post('/transcribe', upload.single("voice"), async (req, res) => {
...
app.get("/report", async (req, res) => {
    ...
    try {
        ...
        try {
            const lump = transcripts.find((t) => t.type === "lump")?.transcript || "";
            const pain = transcripts.find((t) => t.type === "Pain")?.transcript || "";
            const different = transcripts.find((t) => t.type.toLowerCase() === "different")?.transcript || "";
...
app.post("/doctors", upload.single("audio"), async (req, res) => {
...
app.post("/nearby_clinics", async (req, res) => {
    ...
    try {
        ...
        const output = data.elements.map((el) => {
